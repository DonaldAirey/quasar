...
namespace MarkThree.Guardian.Client
{
    public class VolumeHelper
    {
        ...
        /// <summary>
        /// Computes the % of the expected (interpolated) volume compared to the currentVolume.
        /// </summary>
        /// <param name="now">Time of day used to compute the interpolated volume</param>
        /// <param name="averageDailyVolume">The average daily volume for the stock</param>
        /// <param name="currentVolume">The current volume of the stock</param>
        /// <returns></returns>
        public static int CurrentVolumePercentage(DateTime now, decimal averageDailyVolume, decimal currentVolume)
        {
            decimal projectedVolume = ProjectedVolume(now, averageDailyVolume);

            int currentVolumePercentage = 0;
            
            if (projectedVolume != 0)
                currentVolumePercentage = (int)((100 * ((decimal)currentVolume - (decimal)projectedVolume)) / (decimal)projectedVolume);
