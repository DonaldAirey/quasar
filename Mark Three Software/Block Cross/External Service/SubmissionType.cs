...
namespace MarkThree.Guardian.External
{
    ...
    public class SubmissionType
    {
        ...
        static SubmissionType()
        {
            // The table must be locked before the indices can be read into an accelerator array.
            ServerMarketData.SubmissionTypeLock.AcquireReaderLock(System.Threading.Timeout.Infinite);
            ...
            SubmissionType.externalKeyArray = new DataView[] {
                    submissionTypeTable.KeySubmissionTypeExternalId0,
                    submissionTypeTable.KeySubmissionTypeExternalId1};
            // The table must be released after the array is constructed.
            ServerMarketData.SubmissionTypeLock.ReleaseReaderLock();
        }
        ...
        /// <summary>Finds a a SubmissionType record using a configuration and an external identifier.</summary>
        /// <param name="configurationId">Specified which mappings (user id columns) to use when looking up external identifiers.</param>
        /// <param name="parameterId">The name of the parameter as specified in the configuration table.</param>
        ...
        public static int FindKey(object configurationId, string parameterId, object externalId)
        {
            ...
            int externalKeyIndex = SubmissionType.GetExternalKeyIndex(configurationId, parameterId);
            System.Data.DataView externalKeyView = SubmissionType.externalKeyArray[externalKeyIndex];
            ...
            return ((int)(externalKeyView[recordIndex].Row[submissionTypeTable.SubmissionTypeCodeColumn]));
        }
        ...
        public static int FindRequiredKey(object configurationId, string parameterId, string externalId)
        {
            ...
            int internalId = SubmissionType.FindKey(configurationId, parameterId, externalId);
            if ((internalId == int.MinValue))
            {
                throw new Exception(string.Format("The SubmissionType table does not have a record identified by \'{0}\'", externalId));
            }
        ...
        public static object FindOptionalKey(object configurationId, string parameterId, object externalId)
        {
            ...
            if ((externalId != null))
            {
                internalId = SubmissionType.FindKey(configurationId, parameterId, ((string)(externalId)));
        ...
        public static void Load(AdoTransaction adoTransaction)
        {
            ...
            MarkThree.Guardian.Core.SubmissionType.Insert(adoTransaction);
            MarkThree.Guardian.Core.SubmissionType.Update(adoTransaction);
        ...
        {
            // Call the internal methods to lock the tables required for an insert or update operation.
            MarkThree.Guardian.Core.SubmissionType.Delete(adoTransaction);
            // These table lock(s) are required for the 'Delete' operation.
            adoTransaction.LockRequests.AddReaderLock(ServerMarketData.SubmissionTypeLock);
            adoTransaction.LockRequests.AddReaderLock(ServerMarketData.ConfigurationLock);
        }
        
        /// <summary>Deletes a SubmissionType record using Metadata Parameters.</summary>
        /// <param name="transaction">Contains the parameters and exceptions for this command.</param>
        public static void Delete(ParameterList parameters)
        {
            ...
            rowVersion = ((long)(submissionTypeRow[submissionTypeTable.RowVersionColumn]));
            // Call the internal method to complete the operation.
            MarkThree.Guardian.Core.SubmissionType.Delete(adoTransaction, sqlTransaction, rowVersion, submissionTypeCode);
        }
        ...
        {
            // Call the internal methods to lock the tables required for an insert or update operation.
            MarkThree.Guardian.Core.SubmissionType.Archive(adoTransaction);
