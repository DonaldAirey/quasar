namespace MarkThree.Guardian.Forms
{
	...
	/// <summary>
	/// Summary description for MatchHistoryViewer.
	/// </summary>
	public class MatchHistoryViewer : MarkThree.Forms.SpreadsheetViewer
	{
		...
		private System.Media.SoundPlayer soundPlayerAccept;

		/// <summary>
		/// Constructor for the MatchHistoryViewer
		/// </summary>
		public MatchHistoryViewer()
		{
			...
			// This sound is played when an item is accepted.
			System.Reflection.Assembly assembly = Assembly.GetAssembly(typeof(MatchHistoryViewer));
			this.soundPlayerAccept = new SoundPlayer(assembly.GetManifestResourceStream("MarkThree.Guardian.Forms.Accept.wav"));
			this.soundPlayerAccept.LoadCompleted += new AsyncCompletedEventHandler(soundPlayerAccept_LoadCompleted);
		...
		protected override void OpenCommand()
		{
			...
			try
			{
				...
				ClientMarketData.StylesheetRow stylesheetRow = this.blotterRow.IsMatchHistoryStylesheetIdNull() ? null :
					ClientMarketData.Stylesheet.FindByStylesheetId(this.blotterRow.MatchHistoryStylesheetId);
			...
			if (matchRowChangeEvent.Action == DataRowAction.Commit)
			{
				...
				else
				{
					...
					if (matchRow.HasVersion(DataRowVersion.Original))
					{
						...
						// Status Field
						int originalStatusCode = (int)matchRow[ClientMarketData.Match.StatusCodeColumn, DataRowVersion.Original];
						int currentStatusCode = (int)matchRow[ClientMarketData.Match.StatusCodeColumn, DataRowVersion.Current];
						if (originalStatusCode != currentStatusCode)
						{
							fields[Field.Status] = true;
							if (currentStatusCode == Status.Accepted)
								Invoke(new EventHandler(PlayAcceptSound), new object[] { this, EventArgs.Empty });
		...
		private void PlayAcceptSound(object sender, EventArgs eventArgs)
		{

			this.soundPlayerAccept.LoadAsync();
		...
		private void ChangeNegotiationRow(object sender, ClientMarketData.NegotiationRowChangeEvent negotiationRowChangeEvent)
		{
			...
			// Several events are generated as the row is processed.  This trigger is only interested in the rows as they're being
			// comitted to the data model.
			if (negotiationRowChangeEvent.Action == DataRowAction.Commit)
			{

				// Extract the modified row from the event parameters.
				ClientMarketData.NegotiationRow negotiationRow = negotiationRowChangeEvent.Row;

				// This next section will determine whether the row was added or modified.
				if (negotiationRowChangeEvent.Row.HasVersion(DataRowVersion.Original))
				{
					...
					// Status Field
					if (!negotiationRow[ClientMarketData.Negotiation.ExecutionIdColumn, DataRowVersion.Original].Equals(
						negotiationRow[ClientMarketData.Negotiation.ExecutionIdColumn, DataRowVersion.Current]))
					{
					...
					// other change instructions to create fragments that will update only the requested fields in the viewer.
					if (fields)
						if (Hierarchy.IsDescendant(this.blotterRow.ObjectRow, negotiationRow.MatchRow.WorkingOrderRow.BlotterRow.ObjectRow))
							this.fragmentList.Add(DataAction.Update, negotiationRow.MatchRow, fields);

				}
				else
					this.fragmentList.Add(DataAction.Update, negotiationRow.MatchRow, Field.Execution);
...
#else
			// This will display the
			this.XmlDocument = new MatchHistoryDocument(blotter.BlotterId);
#endif
		...
		void soundPlayerAccept_LoadCompleted(object sender, AsyncCompletedEventArgs e)
		{
			...
			try
			{
				...
				this.soundPlayerAccept.PlaySync();
