...
*
*	File:			ArchiveLock.cs
*	Description:	Collects the locks for the External Archive Method.
...
namespace MarkThree.MiddleTier
{
	...
	public class ArchiveLock : ExternalInterfaceMethod
	{

		/// <summary>
		/// Creates a procedure that collects table locks needed for the 'Archive' operation.
		/// </summary>
		/// <param name="externalInterfaceClass"></param>
		public ArchiveLock(ExternalInterfaceSchema ExternalInterfaceSchema, MiddleTierTable middleTierTable)
			: base(ExternalInterfaceSchema, middleTierTable)
		{
			...
			//        {
			this.Comments.Add(new CodeCommentStatement("<summary>Collects the table lock request(s) for an 'Archive' operation</summary>", true));
			...
			//            Shadows.Web.Service.Equity.Archive(transaction);
			...
			this.Statements.Add(new CodeMethodInvokeExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", this.ExternalInterfaceSchema.InternalNamespace, this.MiddleTierTable.ElementTable.Name)), "Archive", new CodeExpression[] {new CodeVariableReferenceExpression("adoTransaction")}));
			...
			this.Statements.Add(new CodeCommentStatement("These table lock(s) are required for the 'Archive' operation."));
