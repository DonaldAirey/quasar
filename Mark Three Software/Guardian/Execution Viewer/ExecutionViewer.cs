namespace MarkThree.Guardian
{
	...
	/// <summary>
	/// Summary description for ExecutionViewer.
	/// </summary>
	public class ExecutionViewer : MarkThree.Forms.SpreadsheetViewer
	{
		...
		private System.Windows.Forms.ContextMenu contextMenuExecution;
		...
		public ExecutionViewer()
		{
			...
			this.ContextMenu = this.contextMenuExecution;
		...
		private void InitializeComponent()
		{
			...
			this.contextMenuExecution = new System.Windows.Forms.ContextMenu();
			...
			this.StylesheetChanged += new System.EventHandler(this.ExecutionViewer_StylesheetChanged);
			this.EndEdit += new MarkThree.Forms.EndEditEventHandler(this.ExecutionViewer_EndEdit);
		...
		/// <summary>
		/// Opens the Block Execution Viewer
		/// </summary>
		protected override void OpenCommand()
		{
			...
			try
			{
				...
				// data, equity traders Equity data, and so forth.  This will pick out the stylesheet used for the Source 
				...
				ClientMarketData.StylesheetRow stylesheetRow = this.Tag is BlotterWorkingOrderDetail ?
					(blotterRow.IsExecutionDetailStylesheetIdNull() ? null :
					ClientMarketData.Stylesheet.FindByStylesheetId(blotterRow.ExecutionDetailStylesheetId)) :
					this.Tag is Blotter ? (blotterRow.IsExecutionStylesheetIdNull() ? null :
					ClientMarketData.Stylesheet.FindByStylesheetId(blotterRow.ExecutionStylesheetId)) : null;
				if (stylesheetRow == null)
					throw new Exception(String.Format("Blotter {0} has no Source Order stylesheet assigned.", this.blotter.BlotterId));
				...
				ClientMarketData.State.StateRowChanging += new ClientMarketData.StateRowChangeEventHandler(ChangeStateRow);
		...
		/// <summary>
		/// Closes the Block Execution Viewer.
		/// </summary>
		...
		/// <summary>
		/// Determines whether a given Source Order belongs on this document.
		/// </summary>
					...
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.DestinationOrderRow destinationOrderRow in workingOrderRow.GetDestinationOrderRows())
							foreach (ClientMarketData.ExecutionRow executionRow in destinationOrderRow.GetExecutionRows())
								fragmentList.Add(DataAction.Update, executionRow, Field.Blotter);
			...
			if (destinationRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.DestinationOrderRow destinationOrderRow in destinationRowChangeEvent.Row.GetDestinationOrderRows())
					if (IsDescendant(destinationOrderRow.WorkingOrderRow))
						foreach (ClientMarketData.ExecutionRow executionRow in destinationOrderRow.GetExecutionRows())
							fragmentList.Add(DataAction.Update, executionRow, Field.Destination);
		...
		/// <param name="sender">The object that generated the event.</param>
		/// <param name="blotterRowChangeEvent">The parameters for the event handler.</param>
		private void ChangeExecutionRow(object sender, ClientMarketData.ExecutionRowChangeEvent executionRowChangeEvent)
		{
			...
			if (executionRowChangeEvent.Action == DataRowAction.Commit)
			{
				...
				if (executionRow.RowState == DataRowState.Deleted)
				{

					int destinationOrderId = (int)executionRow[ClientMarketData.Execution.DestinationOrderIdColumn, DataRowVersion.Original];
					...
					if (IsDescendant(destinationOrderRow.WorkingOrderRow))
						this.fragmentList.Add(DataAction.Delete, executionRow);
				...
				else
				{
					...
					if (executionRow.HasVersion(DataRowVersion.Original))
					{
						...
						int originalBlotterId = (int)executionRow.DestinationOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.BlotterIdColumn,
							DataRowVersion.Original];
						int currentBlotterId = (int)executionRow.DestinationOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.BlotterIdColumn,
						...
						if (originalBlotterId != currentBlotterId)
						{
							...
							else
							{
								...
								if (!isOriginalDescendant && isCurrentDescendant)
									this.fragmentList.Add(DataAction.Insert, executionRow);
								else
								{
									if (isOriginalDescendant && !isCurrentDescendant)
										this.fragmentList.Add(DataAction.Delete, executionRow);
						...
						if (IsDescendant(executionRow.DestinationOrderRow.WorkingOrderRow))
						{

							// If the destination status has changed then give the template a chance to redraw all the fields.
							int originalDestinationStateCode = (int)executionRow[ClientMarketData.Execution.DestinationStateCodeColumn,
								DataRowVersion.Original];
							int currentDestinationStateCode = (int)executionRow[ClientMarketData.Execution.DestinationStateCodeColumn,
								DataRowVersion.Current];
							if (originalDestinationStateCode != currentDestinationStateCode)
								fields = FieldArray.Set;

							// If the source status has changed then give the template a chance to redraw all the fields.
							int originalSourceStateCode = (int)executionRow[ClientMarketData.Execution.SourceStateCodeColumn,
								DataRowVersion.Original];
							int currentSourceStateCode = (int)executionRow[ClientMarketData.Execution.SourceStateCodeColumn,
								DataRowVersion.Current];
							if (originalSourceStateCode != currentSourceStateCode)
								fields = FieldArray.Set;

							// Execution Quantity Field
							if (!executionRow[ClientMarketData.Execution.ExecutionQuantityColumn, DataRowVersion.Original].Equals(
								executionRow[ClientMarketData.Execution.ExecutionQuantityColumn, DataRowVersion.Current]))
							{
								fields[Field.DestinationState] = true;
								fields[Field.SourceState] = true;
								fields[Field.ExecutionQuantity] = true;
							}

							// Security Field
							if (!executionRow.DestinationOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.SecurityIdColumn, DataRowVersion.Original].Equals(
								executionRow.DestinationOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.SecurityIdColumn, DataRowVersion.Current]))
							{
								fields[Field.DestinationState] = true;
								fields[Field.SourceState] = true;
						...
						if (fields)
							this.fragmentList.Add(DataAction.Update, executionRow, fields);
			...
			if (orderTypeRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in orderTypeRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.DestinationOrderRow destinationOrderRow in workingOrderRow.GetDestinationOrderRows())
							foreach (ClientMarketData.ExecutionRow executionRow in destinationOrderRow.GetExecutionRows())
								fragmentList.Add(DataAction.Update, executionRow, Field.OrderType);
			...
			if (orderTypeRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in orderTypeRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.DestinationOrderRow destinationOrderRow in workingOrderRow.GetDestinationOrderRows())
							foreach (ClientMarketData.ExecutionRow executionRow in destinationOrderRow.GetExecutionRows())
								fragmentList.Add(DataAction.Update, executionRow, Field.PriceType);
		}
		...
		/// <summary>
		/// Event driver for a change to the state table.
		/// </summary>
		/// <param name="sender">The object that generated the event.</param>
		/// <param name="stateRowChangeEvent">The parameters for the event handler.</param>
		private void ChangeStateRow(object sender, ClientMarketData.StateRowChangeEvent stateRowChangeEvent)
		{

			// When the destination state has changed, every row in the report containing a relation to the state item will be updated.
			if (stateRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.ExecutionRow executionRow in stateRowChangeEvent.Row.GetExecutionRowsByStateExecutionDestinationStateCode())
					if (IsDescendant(executionRow.DestinationOrderRow.WorkingOrderRow))
						fragmentList.Add(DataAction.Update, executionRow, Field.DestinationState);

			// When the source state has changed, every row in the report containing a relation to the state item will be updated.
			if (stateRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.ExecutionRow executionRow in stateRowChangeEvent.Row.GetExecutionRowsByStateExecutionSourceStateCode())
					if (IsDescendant(executionRow.DestinationOrderRow.WorkingOrderRow))
						fragmentList.Add(DataAction.Update, executionRow, Field.SourceState);
			...
			if (timeInForceRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in timeInForceRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.DestinationOrderRow destinationOrderRow in workingOrderRow.GetDestinationOrderRows())
							foreach (ClientMarketData.ExecutionRow executionRow in destinationOrderRow.GetExecutionRows())
								fragmentList.Add(DataAction.Update, executionRow, Field.TimeInForce);
		...
		protected override void DrawDocumentCommand(object parameter)
		{
			...
			ExecutionDocument executionDocument = new ExecutionDocument(blotter, workingOrders);
			...
			this.XmlDocument = executionDocument;
#else
			// This will draw the document by passing it through the stylesheet.
			this.XmlDocument = new ExecutionDocument(blotter, workingOrders);
#endif
		...
		private void DrawFragmentCommand(object parameter)
		{
			...
			XmlDocument executionFragment = new ExecutionDocument(fragmentList);
			...
			try
			{
				...
				// During debugging, it's very useful to have a copy of the DOM on the disk.
				executionFragment.Save("executionFragment.xml");
			...
			this.XmlDocument = executionFragment;
#else
			// This will write the incremental changes out to the viewer.
			this.XmlDocument = new ExecutionDocument(fragmentList);
#endif
		...
		private void ExecutionViewer_EndEdit(object sender, MarkThree.Forms.EndEditEventArgs e)
		{
			...
			try
			{
				...
				System.Diagnostics.Debug.Assert(!ClientMarketData.IsLocked);
				ClientMarketData.ExecutionLock.AcquireReaderLock(Timeout.Infinite);

				foreach (object[] key in this.SelectedItems)
				{

					int executionId = (int)key[0];
					...
					if (executionRow != null)
					{
						...
						TransactionPlan transactionPlan = batch.Transactions.Add();
						AssemblyPlan assemblyPlan = batch.Assemblies.Add("Core Service");
						TypePlan typePlan = assemblyPlan.Types.Add("MarkThree.Guardian.Core.Execution");
						MethodPlan methodPlan = transactionPlan.Methods.Add(typePlan, "Update");
						methodPlan.Parameters.Add(new InputParameter("executionId", executionRow.ExecutionId));
						methodPlan.Parameters.Add(new InputParameter("rowVersion", executionRow.RowVersion));
		...
		private void ExecutionViewer_StylesheetChanged(object sender, System.EventArgs e)
		{
