namespace MarkThree.Guardian
{
	...
	/// <summary>
	/// Summary description for SourceOrderViewer.
	/// </summary>
	public class SourceOrderViewer : MarkThree.Forms.SpreadsheetViewer
	{
		...
		private System.Windows.Forms.ContextMenu contextMenuSourceOrder;
		...
		public SourceOrderViewer()
		{
			...
			this.ContextMenu = this.contextMenuSourceOrder;
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(SourceOrderViewer));
			this.contextMenuSourceOrder = new System.Windows.Forms.ContextMenu();
			this.clientMarketData = new MarkThree.Guardian.Client.ClientMarketData(this.components);
			// 
			// contextMenuSourceOrder
			// 
			this.contextMenuSourceOrder.RightToLeft = ((System.Windows.Forms.RightToLeft)(resources.GetObject("contextMenuSourceOrder.RightToLeft")));
			// 
			// SourceOrderViewer
			// 
			...
			this.Name = "SourceOrderViewer";
			this.RightToLeft = ((System.Windows.Forms.RightToLeft)(resources.GetObject("$this.RightToLeft")));
			this.Size = ((System.Drawing.Size)(resources.GetObject("$this.Size")));
			this.StylesheetChanged += new System.EventHandler(this.SourceOrderViewer_StylesheetChanged);
			this.EndEdit += new MarkThree.Forms.EndEditEventHandler(this.SourceOrderViewer_EndEdit);
		...
		protected override void OpenCommand()
		{
			...
			try
			{
				...
				ClientMarketData.StylesheetRow stylesheetRow = this.Tag is BlotterWorkingOrderDetail ?
					(blotterRow.IsSourceOrderDetailStylesheetIdNull() ? null :
					ClientMarketData.Stylesheet.FindByStylesheetId(blotterRow.SourceOrderDetailStylesheetId)) :
					this.Tag is Blotter ? (blotterRow.IsSourceOrderStylesheetIdNull() ? null :
					ClientMarketData.Stylesheet.FindByStylesheetId(blotterRow.SourceOrderStylesheetId)) : null;
				...
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in blotterRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.SourceOrderRow sourceOrderRow in workingOrderRow.GetSourceOrderRows())
							fragmentList.Add(DataAction.Update, sourceOrderRow, Field.Blotter);
		...
		public void ChangeDestinationRow(object sender, ClientMarketData.DestinationRowChangeEvent destinationRowChangeEvent)
		{
			...
			if (destinationRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.SourceOrderRow sourceOrderRow in destinationRowChangeEvent.Row.GetSourceOrderRows())
					if (IsDescendant(sourceOrderRow.WorkingOrderRow))
						fragmentList.Add(DataAction.Update, sourceOrderRow, Field.Destination);
		...
		/// <param name="sender">The object that generated the event.</param>
		/// <param name="blotterRowChangeEvent">The parameters for the event handler.</param>
		private void ChangeSourceOrderRow(object sender, ClientMarketData.SourceOrderRowChangeEvent sourceOrderRowChangeEvent)
		{
			...
			if (sourceOrderRowChangeEvent.Action == DataRowAction.Commit)
			{

				// Extract the modified row from the event parameters.
				ClientMarketData.SourceOrderRow sourceOrderRow = sourceOrderRowChangeEvent.Row;
				...
				if (sourceOrderRow.RowState == DataRowState.Deleted)
				{

					int workingOrderId = (int)sourceOrderRow[ClientMarketData.SourceOrder.WorkingOrderIdColumn, DataRowVersion.Original];
					ClientMarketData.WorkingOrderRow workingOrderRow = ClientMarketData.WorkingOrder.FindByWorkingOrderId(workingOrderId);
					if (IsDescendant(workingOrderRow))
						this.fragmentList.Add(DataAction.Delete, sourceOrderRow);
				...
				else
				{
					...
					if (sourceOrderRow.HasVersion(DataRowVersion.Original))
					{
						...
						int originalBlotterId = (int)sourceOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.BlotterIdColumn,
							DataRowVersion.Original];
						int currentBlotterId = (int)sourceOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.BlotterIdColumn,
						...
						if (originalBlotterId != currentBlotterId)
						{
							...
							else
							{
								...
								if (!isOriginalDescendant && isCurrentDescendant)
									this.fragmentList.Add(DataAction.Insert, sourceOrderRow);
								else
								{
									if (isOriginalDescendant && !isCurrentDescendant)
										this.fragmentList.Add(DataAction.Delete, sourceOrderRow);
						...
						if (IsDescendant(sourceOrderRow.WorkingOrderRow))
						{
							...
							int originalStatusCode = (int)sourceOrderRow[ClientMarketData.SourceOrder.StatusCodeColumn,
								DataRowVersion.Original];
							int currentStatusCode = (int)sourceOrderRow[ClientMarketData.SourceOrder.StatusCodeColumn,
								DataRowVersion.Current];
							if (originalStatusCode != currentStatusCode)
								fields = FieldArray.Set;

							// Ordered Quantity Field
							if (!sourceOrderRow[ClientMarketData.SourceOrder.OrderedQuantityColumn, DataRowVersion.Original].Equals(
								sourceOrderRow[ClientMarketData.SourceOrder.OrderedQuantityColumn, DataRowVersion.Current]))
							{
							...
							// OrderType Field
							if (!sourceOrderRow[ClientMarketData.SourceOrder.OrderTypeCodeColumn, DataRowVersion.Original].Equals(
								sourceOrderRow[ClientMarketData.SourceOrder.OrderTypeCodeColumn, DataRowVersion.Current]))
							{
								fields[Field.Status] = true;
							...
							// TimeInForce Field
							if (!sourceOrderRow[ClientMarketData.SourceOrder.TimeInForceCodeColumn, DataRowVersion.Original].Equals(
								sourceOrderRow[ClientMarketData.SourceOrder.TimeInForceCodeColumn, DataRowVersion.Current]))
							{
								fields[Field.Status] = true;
							...
							// Security Field
							if (!sourceOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.SecurityIdColumn, DataRowVersion.Original].Equals(
								sourceOrderRow.WorkingOrderRow[ClientMarketData.WorkingOrder.SecurityIdColumn, DataRowVersion.Current]))
							{
								fields[Field.Status] = true;
						...
						if (fields)
							this.fragmentList.Add(DataAction.Update, sourceOrderRow, fields);
		...
		private void ChangeOrderTypeRow(object sender, ClientMarketData.OrderTypeRowChangeEvent orderTypeRowChangeEvent)
		{
			...
			if (orderTypeRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in orderTypeRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.SourceOrderRow sourceOrderRow in workingOrderRow.GetSourceOrderRows())
							fragmentList.Add(DataAction.Update, sourceOrderRow, Field.OrderType);
		...
		private void ChangePriceTypeRow(object sender, ClientMarketData.PriceTypeRowChangeEvent orderTypeRowChangeEvent)
		{
			...
			if (orderTypeRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in orderTypeRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.SourceOrderRow sourceOrderRow in workingOrderRow.GetSourceOrderRows())
							fragmentList.Add(DataAction.Update, sourceOrderRow, Field.PriceType);
		}
		...
		private void ChangeStatusRow(object sender, ClientMarketData.StatusRowChangeEvent statusRowChangeEvent)
		{
			...
			if (statusRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.SourceOrderRow sourceOrderRow in statusRowChangeEvent.Row.GetSourceOrderRows())
					if (IsDescendant(sourceOrderRow.WorkingOrderRow))
						fragmentList.Add(DataAction.Update, sourceOrderRow, Field.Status);
		...
		private void ChangeTimeInForceRow(object sender, ClientMarketData.TimeInForceRowChangeEvent timeInForceRowChangeEvent)
		{
			...
			if (timeInForceRowChangeEvent.Action == DataRowAction.Commit)
				foreach (ClientMarketData.WorkingOrderRow workingOrderRow in timeInForceRowChangeEvent.Row.GetWorkingOrderRows())
					if (IsDescendant(workingOrderRow))
						foreach (ClientMarketData.SourceOrderRow sourceOrderRow in workingOrderRow.GetSourceOrderRows())
							fragmentList.Add(DataAction.Update, sourceOrderRow, Field.TimeInForce);
		...
		protected override void DrawDocumentCommand(object parameter)
		{
			...
			if (parameter is Blotter || parameter is BlotterMatchDetail)
			{
			...
			SourceOrderDocument sourceOrderDocument = new SourceOrderDocument(blotter, workingOrders);
			...
			try
			{
				...
				sourceOrderDocument.Save("sourceOrderDOM.xml");
			...
			this.XmlDocument = sourceOrderDocument;
#else
			// This will draw the document by passing it through the stylesheet.
			this.XmlDocument = new SourceOrderDocument(blotter, workingOrders);
#endif
		...
		private void DrawFragmentCommand(object parameter)
		{
			...
			XmlDocument sourceOrderFragment = new SourceOrderDocument(fragmentList);
			...
			try
			{
				...
				// During debugging, it's very useful to have a copy of the DOM on the disk.
				sourceOrderFragment.Save("sourceOrderFragment.xml");
			...
			this.XmlDocument = sourceOrderFragment;
#else
			// This will write the incremental changes out to the viewer.
			this.XmlDocument = new SourceOrderDocument(fragmentList);
#endif
		...
		private void SourceOrderViewer_EndEdit(object sender, MarkThree.Forms.EndEditEventArgs e)
		{
			...
			try
			{
				...
				System.Diagnostics.Debug.Assert(!ClientMarketData.IsLocked);
				ClientMarketData.SourceOrderLock.AcquireReaderLock(Timeout.Infinite);

				foreach (object[] key in this.SelectedItems)
				{

					int sourceOrderId = (int)key[0];

					ClientMarketData.SourceOrderRow sourceOrderRow = ClientMarketData.SourceOrder.FindBySourceOrderId(sourceOrderId);
					if (sourceOrderRow != null)
					{
						...
						TransactionPlan transactionPlan = batch.Transactions.Add();
						AssemblyPlan assemblyPlan = batch.Assemblies.Add("Core Service");
						TypePlan typePlan = assemblyPlan.Types.Add("MarkThree.Guardian.Core.SourceOrder");
						MethodPlan methodPlan = transactionPlan.Methods.Add(typePlan, "Update");
						methodPlan.Parameters.Add(new InputParameter("sourceOrderId", sourceOrderRow.SourceOrderId));
						methodPlan.Parameters.Add(new InputParameter("rowVersion", sourceOrderRow.RowVersion));
		...
		private void SourceOrderViewer_StylesheetChanged(object sender, System.EventArgs e)
		{
