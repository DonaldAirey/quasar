namespace MarkThree.MiddleTier.TableClass
{
	...
	public class Insert : CodeMemberMethod
	{
		...
		public Insert(TableSchema tableSchema)
		{
			...
			this.Comments.Add(new CodeCommentStatement(string.Format("Inserts a {0} record.", tableSchema.Name), true));
			this.Comments.Add(new CodeCommentStatement("</summary>", true));
			...
			foreach (ColumnSchema columnSchema in tableSchema.Columns)
				if (!tableSchema.IsPrimaryKeyColumn(columnSchema) || columnSchema.DeclaringType == tableSchema.TypeSchema)
				{
					...
					Type typeVariable = columnSchema.MinOccurs == 0 ? typeof(object) : typeColumn;
					...
					if (tableSchema.IsAutogeneratedColumn(columnSchema))
			...
			//				}
			foreach (KeyrefSchema parentKey in tableSchema.ParentKeyrefs)
				if (!tableSchema.IsBaseKeyref(parentKey))
				{
					...
					lockParentRow.Add(new CodeVariableDeclarationStatement(new CodeTypeReference(string.Format("{0}.{1}Row", tableSchema.DataModelSchema.Name, parentTable.Name)), parentRowName, new CodeMethodInvokeExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", tableSchema.DataModelSchema.Name, parentTable.Name)), methodName, keyList.ToArray())));
			...
			foreach (ColumnSchema columnSchema in tableSchema.Columns)
				if (columnSchema.MinOccurs == 0)
				{
					...
					CodeExpression defaultValueExpression = columnSchema.DefaultValue == null ?
						(CodeExpression)new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(typeof(DBNull)), "Value") :
						(CodeExpression)new CodePrimitiveExpression(columnSchema.DefaultValue);
					this.Statements.Add(new CodeConditionStatement(new CodeBinaryOperatorExpression(new CodeArgumentReferenceExpression(Generate.CamelCase(columnSchema.Name)), CodeBinaryOperatorType.IdentityEquality, new CodePrimitiveExpression(null)), new CodeAssignStatement(new CodeArgumentReferenceExpression(Generate.CamelCase(columnSchema.Name)), defaultValueExpression)));
				}
			...
			foreach (TableSchema familyTable in tableSchema.TableHierarchy)
			{
				...
				this.Statements.Add(new CodeVariableDeclarationStatement(string.Format("{0}.{1}Row", tableSchema.DataModelSchema.Name, familyTable.Name), rowName, new CodeMethodInvokeExpression(new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(tableSchema.DataModelSchema.Name), familyTable.Name), string.Format("New{0}Row", familyTable.Name))));
				...
				foreach (ColumnSchema columnSchema in familyTable.Columns)
					if (columnSchema.DeclaringType == familyTable.TypeSchema)
					{
						...
						if (familyTable.IsInheritedKey(columnSchema))
						{
							...
							tryFinallyStatement.TryStatements.Add(new CodeAssignStatement(new CodeIndexerExpression(new CodeVariableReferenceExpression(rowName), new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", tableSchema.DataModelSchema.Name, familyTable.Name)), string.Format("{0}Column", columnSchema.Name))),
								new CodeIndexerExpression(new CodeVariableReferenceExpression(string.Format("{0}Row", Generate.CamelCase(baseTable.Name))), new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", tableSchema.DataModelSchema.Name, baseTable.Name)), string.Format("{0}Column", baseTable.PrimaryKey.Fields[keyIndex].Name)))
								));
						}
						else
				...
				tryFinallyStatement.TryStatements.Add(new CodeMethodInvokeExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", tableSchema.DataModelSchema.Name, familyTable.Name)), string.Format("Add{0}Row", familyTable.Name), new CodeVariableReferenceExpression(rowName)));
			...
			foreach (ColumnSchema columnSchema in tableSchema.Columns)
				if (tableSchema.IsAutogeneratedColumn(columnSchema) && columnSchema.DeclaringType == tableSchema.TypeSchema)
					this.Statements.Add(new CodeAssignStatement(new CodeArgumentReferenceExpression(Generate.CamelCase(columnSchema.Name)), new CodeCastExpression(columnSchema.DataType, new CodeIndexerExpression(new CodeVariableReferenceExpression(string.Format("{0}Row", Generate.CamelCase(tableSchema.Name))), new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(string.Format("{0}.{1}", tableSchema.DataModelSchema.Name, tableSchema.Name)), string.Format("{0}Column", columnSchema.Name))))));
