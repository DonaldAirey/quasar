...
<xsl:stylesheet version="1.0" xmlns="urn:schemas-microsoft-com:office:spreadsheet"
	...
	<xsl:template match="Appraisal">
			...
			<ss:Workbook xmlns:o="urn:schemas-microsoft-com:office:office"
				...
				<ss:Names>
					...
					<ss:NamedRange ss:Name="securityName" ss:RefersTo="=Sheet1!C11"/>
				...
				<ss:Styles>
					...
					<Style ss:ID="Violated">
						<ss:Borders>
							<ss:Border ss:Position="Bottom" ss:LineStyle="Dot" ss:Weight="2" ss:Color="#FF0000"/>
						</ss:Borders>
					</Style>
					<Style ss:ID="ViolatedShort">
						<ss:Font ss:FontName="Microsoft Sans Serif" ss:Size="8" ss:Color="#FF0000" ss:Bold="0"
						ss:Italic="0" ss:Underline="None" />
						<ss:Borders>
							<ss:Border ss:Position="Bottom" ss:LineStyle="Dot" ss:Weight="2" ss:Color="#FF0000"/>
		...
		<ss:Row ss:Height="12">
			...
			<ss:Cell ss:Index="11">
				<xsl:if test="@PositionTypeCode=1">
					<xsl:attribute name="ss:StyleID">Short</xsl:attribute>
				</xsl:if>
				<ss:Data ss:Type="String"><xsl:value-of select="@Name"/></ss:Data>
			</ss:Cell>
	...
	<xsl:template match="Equity">
		...
		<xsl:variable name="orderQuantity" select="sum(.//Account/@OrderQuantity)"/>
		<xsl:variable name="allocationQuantity" select="sum(.//Account/@AllocationQuantity)"/>
		<xsl:variable name="violationCount" select="sum(.//Account/@Violation)"/>
		<xsl:variable name="subtotalRow" select="1+count(../*)-position()"/>
		...
		<ss:Row ss:Height="12">
			...
			<ss:Cell ss:Index="11">
				<xsl:choose>
					<xsl:when test="@PositionTypeCode=0 and $violationCount!=0">
						<xsl:attribute name="ss:StyleID">Violated</xsl:attribute>
					</xsl:when>
					<xsl:when test="@PositionTypeCode=1 and $violationCount=0">
						<xsl:attribute name="ss:StyleID">Short</xsl:attribute>
					</xsl:when>
					<xsl:when test="@PositionTypeCode=1 and $violationCount!=0">
						<xsl:attribute name="ss:StyleID">ViolatedShort</xsl:attribute>
					</xsl:when>
				</xsl:choose>
				<ss:Data ss:Type="String"><xsl:value-of select="@Name"/></ss:Data>
			</ss:Cell>
