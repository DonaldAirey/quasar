...
namespace Shadows.Quasar.Viewers.Order
{
				...
				switch (xmlNodeReader.NodeType)
				{
					...
					switch (elementNameTable[xmlNodeReader.Name])
					{
						...
						case ElementName.Data:
							...
							if (rowType == RowType.LocalOrder && ColumnIndex == GetColumnIndex(ColumnType.OrderId))
							{
								parsingState = ParsingState.LocalOrderId;
								break;
							}

							// If the current column is a position type code on a position row, then look for the position
							// type code the next time a Text node is found.
							if (rowType == RowType.GlobalOrder && ColumnIndex == GetColumnIndex(ColumnType.OrderId))
							{
								parsingState = ParsingState.GlobalOrderId;
								break;
							}
					...
					switch (parsingState)
					{
						...
						case ParsingState.LocalOrderId:

							// The position type code is found in this cell.
							int localOrderId = Convert.ToInt32(xmlNodeReader.Value);
							this.LocalMap.AddLocalMapRow(localOrderId, documentVersion, rowIndex);

							break;

						case ParsingState.GlobalOrderId:

							// The position type code is found in this cell.
							int globalOrderId = Convert.ToInt32(xmlNodeReader.Value);
							this.GlobalMap.AddGlobalMapRow(globalOrderId, documentVersion, rowIndex);
	...
	internal enum ParsingState {None, RowType, LocalOrderId, GlobalOrderId, Placeholder};
