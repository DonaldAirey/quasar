...
namespace Shadows.Quasar.Viewers.Placement
{
				...
				switch (xmlNodeReader.NodeType)
				{
					...
					switch (elementNameTable[xmlNodeReader.Name])
					{
						...
						case ElementName.Data:
							...
							if (rowType == RowType.Placement && columnIndex == GetColumnIndex(ColumnType.PlacementId))
							{
								parsingState = ParsingState.Placement;
								break;
							}
							...
							if (rowType == RowType.Placeholder)
							{
					...
					switch (parsingState)
					{
						...
						case ParsingState.Placement:

							// The position type code is found in this cell.
							int localPlacementId = Convert.ToInt32(xmlNodeReader.Value);
							int mappedColumnIndex;

							mappedColumnIndex = GetColumnIndex(ColumnType.RowType);
							if (mappedColumnIndex == SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.RowType, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.BrokerId);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.BrokerId, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.BrokerName);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.BrokerName, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.BrokerSymbol);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.BrokerSymbol, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.PlacementId);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.PlacementId, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.Quantity);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.Quantity, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.TimeInForceCode);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.TimeInForceCode, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.TimeInForceMnemonic);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.TimeInForceMnemonic, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.OrderTypeCode);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.OrderTypeCode, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.OrderTypeMnemonic);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.OrderTypeMnemonic, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.RowVersion);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.RowVersion, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.Price1);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.Price1, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.Price2);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.Price2, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.CreatedTime);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.CreatedTime, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.CreatedLoginId);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.CreatedLoginId, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.CreatedLoginName);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.CreatedLoginName, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.ModifiedTime);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.ModifiedTime, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.ModifiedLoginId);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.ModifiedLoginId, documentVersion, rowIndex, mappedColumnIndex);

							mappedColumnIndex = GetColumnIndex(ColumnType.ModifiedLoginName);
							if (mappedColumnIndex != SpreadsheetColumn.DoesNotExist)
								this.AddressMap.AddAddressMapRow(localPlacementId, ColumnType.ModifiedLoginName, documentVersion, rowIndex, mappedColumnIndex);
	...
	internal enum ParsingState {None, RowType, Placement, Placeholder};
