namespace MarkThree.Quasar.Server
{
	...
	/// <summary>The server data model for the Quasar Crossing Network.</summary>
	public class ServerDataModel : DataModel
	{

		// Private Static Members
		private static string PersistentStore = "Quasar";
		...
		/// <summary>
		/// Constructor for a ServerDataModel when used as a component.
		/// </summary>
		public ServerDataModel() : base() { }

		/// <summary>
		/// Constructor for a ServerDataModel when used as a component.
		/// </summary>
		/// <param name="container">The container object.</param>
		public ServerDataModel(System.ComponentModel.IContainer container) : base(container) { }
		...
		public static RowVersion RowVersion { get { return ServerDataModel.rowVersion; } }
		...
		public static int UserId
		{
			...
			get
			{
				...
				try
				{
					...
					ServerDataModel.UserLock.AcquireReaderLock(CommonTimeout.LockWait);

					// Use the unique index to find a match to the aliased user name from the environment.
					int index = ServerDataModel.User.KeyUserUserName.Find(new object[] { System.Environment.UserName });
					if (index == -1)
						throw new Exception(string.Format("The user '{0}' is not mapped to a User", System.Environment.UserName));
					return (int)ServerDataModel.User.KeyUserUserName[index].Row[ServerDataModel.User.UserIdColumn];
				...
				finally
				{
					...
					ServerDataModel.UserLock.ReleaseReaderLock();
		...
		public static bool FilterObject(DataRow userDataRow, DataRow objectDataRow)
		{
			...
			ServerDataModel.ObjectRow objectRow = (ServerDataModel.ObjectRow)objectDataRow;
		...
		public static bool FilterSecurity(DataRow userDataRow, DataRow securityDataRow)
		{
			...
			// if the record should not be included in the user's data model.
			ServerDataModel.UserRow userRow = (ServerDataModel.UserRow)userDataRow;
			ServerDataModel.SecurityRow securityRow = (ServerDataModel.SecurityRow)securityDataRow;
		...
		public static bool FilterCurrency(DataRow userDataRow, DataRow currencyDataRow)
		{
			...
			// if the record should not be included in the user's data model.
			ServerDataModel.UserRow userRow = (ServerDataModel.UserRow)userDataRow;
			ServerDataModel.CurrencyRow currencyRow = (ServerDataModel.CurrencyRow)currencyDataRow;
		...
		public static void Reconcile(AdoTransaction adoTransaction)
		{
			...
			// In order to capture the referential intergrity of the ServerDataModel, we need to prevent any updates to
			// the DataSet until we're through scanning it.
			foreach (Table table in ServerDataModel.Tables)
				adoTransaction.LockRequests.Add(new TableReaderRequest(table));
		...
		public static void SetMask(AdoTransaction adoTransaction)
		{
			...
			try
			{
				...
				ServerDataModel.maskLock.AcquireReaderLock(CommonTimeout.LockWait);

				Table table = (Table)ServerDataModel.maskLocks[MarkThree.Quasar.Server.Environment.UserName];
				adoTransaction.LockRequests.Add(new TableWriterRequest(table));

			}
			finally
			{

				if (ServerDataModel.maskLock.IsReaderLockHeld)
					ServerDataModel.maskLock.ReleaseReaderLock();

			}

			// HACK - Fix User Mask logic.
//			adoTransaction.LockRequests.Add(new TableWriterRequest(ServerDataModel.maskLock));
		...
		public static void SetMask(ParameterList parameters)
		{
			...
			string userName = MarkThree.Quasar.Server.Environment.UserName;

			DataSet sourceMask = parameters["maskSet"];

			DataSet destinationMask = (DataSet)ServerDataModel.masks[userName];

			if (destinationMask == null)
			{
				destinationMask = new DataSet(string.Format("{0} Mask", userName));
				ServerDataModel.masks[userName] = destinationMask;
			}
		...
		public static void MergeMask(AdoTransaction adoTransaction)
		{
			...
			// HACK - Fix user mask logic for new table locking scheme.
//			adoTransaction.LockRequests.Add(new TableWriterRequest(ServerDataModel.mask));
