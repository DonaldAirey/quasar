namespace MarkThree.Forms
{
	...
	public class XmlStylesheetReader : XmlReader
	{
		...
		/// <summary>
		/// Read the next node in the XML structure of the stylesheet.
		/// </summary>
		/// <returns>true if the next node was read successfully; false if there are no more nodes to read.</returns>
		public override bool Read()
		{

			// The stylesheet is organized as a tree structure of parent nodes and children.  The idea here is to transverse the
			...
			if (this.stateStack.Count == 0)
			{
				...
				// Initialize the stack with the root stylesheet node.
				Node node = this.stylesheet;
				this.stateStack.Push(new NodeState(node, node.GetEnumerator()));
			...
			else
			{
				...
				// The state of reading the Stylesheet as an XML stream is kept on the 'stateStack'.  Each time another 'Read' is
